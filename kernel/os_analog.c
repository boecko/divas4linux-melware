...
**  LOCALS
*/
static unsigned long _analog_bar_length[4] = {
  512,
  0x100, /* I/O */
  MQ2_MEMORY_SIZE,
  256*1024
...
static unsigned long _analog_pcie_bar_length[4] = {
  512,
  0, /* not supported */
  MP2_MEMORY_SIZE,
  256*1024
...
static int diva_analog_cleanup_adapter (diva_os_xdi_adapter_t* a);
static int diva_analog_stop_no_io (diva_os_xdi_adapter_t* a);
static int diva_analog_cmd_card_proc (struct _diva_os_xdi_adapter* a,
                                    diva_xdi_um_cfg_cmd_t* cmd,
                                    int length);
static int diva_analog_write_fpga_image (diva_os_xdi_adapter_t* a,
                                       byte* data,
                                       dword length);
static int diva_analog_reset_adapter (struct _diva_os_xdi_adapter* a);
static int diva_analog_write_sdram_block (PISDN_ADAPTER IoAdapter,
                                        dword address,
                                        const byte* data,
                                        dword length,
                                        dword limit);
...
**  BAR2 - MEM - MQ2_MEMORY_SIZE - SDRAM
**  BAR3 - MEM - 256KBytes       - CNTRL
**
**  Called by master adapter, that will initialize and add slave adapters
*/
int
diva_analog_init_card (diva_os_xdi_adapter_t* a)
{
  int bar;
  unsigned long bar_length[sizeof(_analog_bar_length)/sizeof(_analog_bar_length[0])];

	if (DIVA_ANALOG_PCIE(&a->xdi_adapter) == 0) {
		memcpy (bar_length, _analog_bar_length, sizeof(bar_length));
	} else {
		memcpy (bar_length, _analog_pcie_bar_length, sizeof(_analog_pcie_bar_length));
	}
    ...
    */
  {
    ...
    DBG_LOG(("A(%d) read Subsystem ID %04x", a->xdi_adapter.ANum, id))
		...
		if (a->CardOrdinal == CARDTYPE_DIVASRV_ANALOG_8PORT) {
			switch (id) {
				case 0x2800:
					a->CardOrdinal = CARDTYPE_DIVASRV_V_ANALOG_8PORT;
					break;
			}
		...
		} else if (a->CardOrdinal == CARDTYPE_DIVASRV_ANALOG_2PORT) {
			switch (id) {
				case 0x2200:
  ...
  /*
    Get Serial Number
    The serial number of PLX is accessible in accordance with PCI spec
  ...
  */
    ...
    DBG_ERR((" %s, failed to read serial number", a->xdi_adapter.Properties.Name))
  ...
  */
  for (bar = 0; bar < 4; bar++) {
    if (bar != 1) { /* ignore I/O */
      ...
      if (!a->resources.pci.addr[bar]) {
        DBG_ERR(("ANALOG: can't map bar[%d]", bar))
        diva_analog_cleanup_adapter (a);
        return (-1);
      }
    }
  ...
  /*
    Register I/O port
  */
  sprintf (&a->port_name[0], "DIVA ANALOG %ld", (long)a->xdi_adapter.serialNo);

  if (a->resources.pci.bar[1] != 0 &&
			diva_os_register_io_port (a, 1, a->resources.pci.bar[1],
                                bar_length[1],
                                &a->port_name[0])) {
    DBG_ERR(("ANALOG: can't register bar[1]"))
    diva_analog_cleanup_adapter (a);
    return (-1);
  }
  ...
  */
  a->interface.cleanup_adapter_proc = diva_analog_cleanup_adapter;
  a->interface.stop_no_io_proc      = diva_analog_stop_no_io;

  a->xdi_adapter.a.io = &a->xdi_adapter;
  a->xdi_adapter.DIRequest = request;
  a->interface.cmd_proc = diva_analog_cmd_card_proc;
  ...
  if (a->CardOrdinal == CARDTYPE_DIVASRV_ANALOG_2PORT ||
    ...
    strcpy (a->xdi_adapter.req_soft_isr.dpc_thread_name, "kdivas8and");
    a->dsp_mask = 0x000000ff;
  }
  ...
  */
  ...
  a->xdi_adapter.AdapterTestMemoryLength = bar_length[2] - 1;
  ...
  */
  ...
  sprintf (a->xdi_adapter.irq_info.irq_name, "DIVA ANALOG %ld", (long)a->xdi_adapter.serialNo);
...
*/
static int
diva_analog_cleanup_adapter (diva_os_xdi_adapter_t* a)
{
  int bar;
...
static int
diva_analog_cmd_card_proc (struct _diva_os_xdi_adapter* a,
                         diva_xdi_um_cfg_cmd_t* cmd,
                         int length)
{
  int ret = -1;
  ...
  if (cmd->adapter != a->controller) {
    DBG_ERR(("analog_cmd, invalid controller=%d != %d", cmd->adapter, a->controller))
    return (-1);
  }
  ...
  switch (cmd->command) {
    ...
    case DIVA_XDI_UM_CMD_RESET_ADAPTER:
      if (!a->xdi_adapter.ControllerNumber) {
        ret = diva_analog_reset_adapter (a);
      }
      break;

    case DIVA_XDI_UM_CMD_WRITE_SDRAM_BLOCK:
      ret = diva_analog_write_sdram_block (&a->xdi_adapter,
                                           cmd->command_data.write_sdram.offset,
                                           (byte*)&cmd[1],
                                           cmd->command_data.write_sdram.length,
																					 (DIVA_ANALOG_PCIE(&a->xdi_adapter) != 0) ?
																							DIVA_ANALOG_PCIE_REAL_SDRAM_SIZE : a->xdi_adapter.MemorySize);
      break;

    case DIVA_XDI_UM_CMD_START_ADAPTER:
      ret = diva_analog_start_adapter (&a->xdi_adapter,
    ...
    case DIVA_XDI_UM_CMD_ADAPTER_TEST:
      if (a->xdi_adapter.DivaAdapterTestProc) {
				...
				if (DIVA_ANALOG_PCIE(&a->xdi_adapter) == 0) {
					...
					real_memory_size = DIVA_ANALOG_PCIE_REAL_SDRAM_SIZE;
					mapped_memory_size = _analog_pcie_bar_length[2];
					las0ba_ctrl = (volatile dword*)&a->xdi_adapter.reset[0x04];
					las0ba_original = *las0ba_ctrl;
				}
    ...
    case DIVA_XDI_UM_CMD_STOP_ADAPTER:
      ret = diva_analog_stop_adapter (a);
      break;
...
void
diva_os_prepare_analog_functions (PISDN_ADAPTER IoAdapter)
{
}

static int
diva_analog_write_fpga_image (diva_os_xdi_adapter_t* a, byte* data, dword length)
{
  int ret;
  ...
  ret = analog_FPGA_download (&a->xdi_adapter);
...
static int
diva_analog_reset_adapter (struct _diva_os_xdi_adapter* a)
{
  ...
  if (IoAdapter->Initialized) {
    DBG_ERR(("A: A(%d) can't reset ANALOG adapter - please stop first", IoAdapter->ANum))
    return (-1);
  }
...
static int
diva_analog_write_sdram_block (PISDN_ADAPTER IoAdapter,
                               dword address,
                               const byte* data,
                               dword length,
                               dword limit)
{
  if (((address + length) >= limit) || !IoAdapter->Address) {
    DBG_ERR(("A(%d) write ANALOG address=0x%08x limit=0x%08x",
						IoAdapter->ANum, address+length, limit))
    return (-1);
  }

	if (diva_dma_write_sdram_block (IoAdapter, 0, 0, address, data, length) != 0) {
...
static void
diva_analog_clear_interrupts (diva_os_xdi_adapter_t* a)
{
  PISDN_ADAPTER IoAdapter = &a->xdi_adapter;
...
static int
diva_analog_stop_adapter_w_io (diva_os_xdi_adapter_t* a, int do_io)
{
  PISDN_ADAPTER IoAdapter = &a->xdi_adapter;
  int i;
...
static int diva_analog_stop_adapter (diva_os_xdi_adapter_t* a) {
	return (diva_analog_stop_adapter_w_io (a, 1));
}

static int diva_analog_stop_no_io (diva_os_xdi_adapter_t* a) {
	if (a->xdi_adapter.Initialized != 0)
		return (diva_analog_stop_adapter_w_io (a, 0));
