...
/*
 * Sometimes it's necessary to have an own debug output for a module inside
 *  a driver (MTPX inside DIMAINT, ISDNXDI inside DIWAN).
 * The debuglib source contains this feature now.
 * 1. Prepare a compiler macro MYDRIVERDEBUGHANDLE with the name of the
 *  _DbgHandle_ (e.g. /DMYDRIVERDEBUGHANDLE=modulDebugHandle)
 * 2. Define the _DbgHandle_ variable MYDRIVERDEBUGHANDLE global for the module
 *  - _DbgHandle_ MYDRIVERDEBUGHANDLE = { 0, DBG_HANDLE_VERSION };
 *  Don't include "debuglib.c" in the module.
 * 3. Use the "Handle functions" DbgRegisterH, DbgDeregisterH, DbgSetLevelH
 *  and xdi_dbg_xlogH instead of the originals
 * 4. The familiar debug macros will be used - DBG_xyz((...))
 *  The macros use functions, which operate with a global pointer to the
 *  _DbgHandle_ structure. This pointer will be set before the functions are
 *  called. Maybe it's possible to write the trace output to a false handle
 *  on multiproc machines.
 * A macro MODULDRIVERDEBUG is defined to use in module sources.
 */
#ifndef MYDRIVERDEBUGHANDLE
#define MYDRIVERDEBUGHANDLE myDriverDebugHandle
#else
...
#if !defined(__DIVA_TASK_SMP__)
extern _DbgHandle_ MYDRIVERDEBUGHANDLE ;
#endif
extern _DbgHandle_ *pMyDriverDebugHandle ;
...
#define DBG_EXT_TYPE_UM_TRACE       0x00000002
typedef struct
{
    ...
    union
    {
        ...
        /* DBG_EXT_TYPE_UM_TRACE */
        struct
        {
   char *pManagementIfVar ; /* variable path for management interface */
   int   ManagementIfId ;  /* Id (logical number) for management interface */
  } UserModeTrace ;
    }Data;     
} _DbgExtendedInfo_;
/*
 * prototypes for debug register/deregister functions in "debuglib.c"
 */
extern int  DbgRegisterH (_DbgHandle_ *pDebugHandle, char *drvName, char *drvTag, unsigned long dbgMask) ;
extern void DbgDeregisterH (_DbgHandle_ *pDebugHandle) ;
extern void DbgSetLevelH (_DbgHandle_ *pDebugHandle, unsigned long dbgMask) ;
/* -------------------------------------------------------------
